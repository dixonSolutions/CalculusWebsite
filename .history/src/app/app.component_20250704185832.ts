...
export class AppComponent implements OnInit {
  ...
  async ngOnInit() {
    ...
    // Also check for phone authentication state changes
    this.checkPhoneAuthState();
    
    // Set up a periodic check for phone authentication (since it doesn't have event-driven updates)
    setInterval(() => {
      this.checkPhoneAuthState();
  ...
  private checkPhoneAuthState() {
    ...
    if (isPhoneAuth && phoneUser) {
      console.log('Phone authentication active:', phoneUser);
      // Phone user is signed in, redirect to auth page if not already there
      if (this.currentRoute !== 'auth' && this.currentRoute !== 'lessons') {
        this.router.navigate(['/auth']);
      }
    }
  }
