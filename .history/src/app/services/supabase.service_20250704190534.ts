...
export class SupabaseService {
  ...
  async authenticatePhone(phone: string): Promise<{ success: boolean; error?: string }> {
    try {
      console.log('Authenticating phone user and creating Supabase account:', phone);
      
      // Create a phone-based email for Supabase authentication
      const phoneDigits = phone.replace(/[^0-9]/g, '');
      const phoneEmail = `${phoneDigits}@phone.auth.local`;
      
      // First, try to sign in the user (they might already exist)
      console.log('Attempting to sign in existing phone user...');
      let { data: signInData, error: signInError } = await this.supabase.auth.signInWithOtp({
        email: phoneEmail,
        options: {
          shouldCreateUser: false // Don't create if doesn't exist yet
        }
      });

      // If user doesn't exist, create them
      if (signInError && signInError.message.includes('User not found')) {
        console.log('User not found, creating new phone-based Supabase user...');
        
        // Create a new user with phone-based email
        const { data: signUpData, error: signUpError } = await this.supabase.auth.signUp({
          email: phoneEmail,
          password: `phone_${phoneDigits}_${Date.now()}`, // Generate a unique password
          options: {
            data: {
              phone: phone,
              verified_via: 'phone_sms',
        ...
        console.log('Phone user created successfully:', signUpData.user?.email);
        
        // Set the session from sign up
        if (signUpData.session) {
          this._session = signUpData.session;
          console.log('Session set from sign up:', this._session);
        }
        
      } else if (signInError) {
        console.error('Error signing in phone user:', signInError);
        return { success: false, error: `Sign in failed: ${signInError.message}` };
      } else {
        console.log('Existing phone user signed in successfully');
        if (signInData.session) {
          this._session = signInData.session;
          console.log('Session set from sign in:', this._session);
        }
      }
      ...
      if (!this._session) {
        ...
        console.log('Session retrieved:', this._session);
      }
      ...
      console.log('Supabase session created:', this._session.user.email);
