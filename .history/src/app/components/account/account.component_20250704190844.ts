...
export class AccountComponent implements OnInit {
  ...
  getWelcomeMessage(): string {
    const displayName = this.profile?.username || 
                       (this.isPhoneAuth ? this.supabase.getPhoneNumber() : null) || 
                       'User';
    
    if (this.isNewUser) {
      return `🎉 Welcome to Master Calculus! Your ${this.isPhoneAuth ? 'phone' : ''} account has been created successfully.`;
    }
  ...
  getUserEmail(): string {
    if (this.isPhoneAuth) {
      return this.supabase.getPhoneNumber() || 'Phone User';
    }
